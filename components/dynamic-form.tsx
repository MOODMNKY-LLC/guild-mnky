...
const getZodDef = (schema: any): Record<string, any> => (schema as any)?._def || (schema as any)?.def
...
export function DynamicForm<T extends z.ZodRawShape = z.ZodRawShape>({
  ...
  const defaultValues = Object.keys(schema.shape).reduce(
    (acc, key) => {
      const originalFieldSchema: any = (schema as any).shape[key]
    ...
    if (initialValues) {
      ...
      const processedInitialValues = schemaKeys.reduce(
        (acc, key) => {
          ...
          const value = initialValues.hasOwnProperty(key) ? (initialValues as Record<string, any>)[key] : undefined
